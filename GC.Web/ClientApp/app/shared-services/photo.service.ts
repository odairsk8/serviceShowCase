...
export class PhotoService {
    ...
    uploadProvidedServiceThumbnailPhoto(companyId: number, providedServiceId: number, file: any) {
        var formData = new FormData();
        formData.append('file', file);
        const url = `api/company/${companyId}/ProvidedService/${providedServiceId}/Thumbnail`;
        return this.http.post(url, formData)
            .map(m => m.json());
    }

    removeProvidedServiceThumbnailPhoto(companyId: number, providedServiceId: number) {
        const url = `api/company/${companyId}/ProvidedService/${providedServiceId}/Thumbnail`;
        return this.http.delete(url)
            .map(m => m.json());
    }
